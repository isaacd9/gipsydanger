...
abstract class BaseUsers extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'gipsydanger\\peopleBundle\\Model\\UsersPeer';
    ...
    public function setRoleId($v)
    {
        ...
        if ($this->role_id !== $v) {
            $this->role_id = $v;
            $this->modifiedColumns[] = UsersPeer::ROLE_ID;
        }
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(UsersPeer::SALT)) {
            $modifiedColumns[':p' . $index++]  = '`salt`';
        }
        if ($this->isColumnModified(UsersPeer::EMAIL)) {
            $modifiedColumns[':p' . $index++]  = '`email`';
        }
        if ($this->isColumnModified(UsersPeer::IS_ACTIVE)) {
            $modifiedColumns[':p' . $index++]  = '`is_active`';
        }
        if ($this->isColumnModified(UsersPeer::ROLE_ID)) {
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if ($this->aRoles !== null) {
                if (!$this->aRoles->validate($columns)) {
                    $failureMap = array_merge($failureMap, $this->aRoles->getValidationFailures());
                }
            }
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aRoles) {
                $result['Roles'] = $this->aRoles->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UsersPeer::ROLE_ID)) $criteria->add(UsersPeer::ROLE_ID, $this->role_id);
    ...
    /**
     * Declares an association between this object and a Roles object.
     *
     * @param             Roles $v
     * @return Users The current object (for fluent API support)
     * @throws PropelException
     */
    public function setRoles(Roles $v = null)
    {
        if ($v === null) {
            $this->setRoleId(NULL);
        } else {
        ...
        // If this object has already been added to the Roles object, it will not be re-added.
        if ($v !== null) {
            $v->addUsers($this);
        }
    ...
    /**
     * Get the associated Roles object
     *
     * @param PropelPDO $con Optional Connection object.
     * @param $doQuery Executes a query to get the object if required
     * @return Roles The associated Roles object.
     * @throws PropelException
     */
    public function getRoles(PropelPDO $con = null, $doQuery = true)
    {
        if ($this->aRoles === null && ($this->role_id !== null) && $doQuery) {
            $this->aRoles = RolesQuery::create()->findPk($this->role_id, $con);
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->aRoles instanceof Persistent) {
              $this->aRoles->clearAllReferences($deep);
            }
